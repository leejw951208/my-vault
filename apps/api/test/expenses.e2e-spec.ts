...
describe("Expenses & Occurrences e2e", () => {
    ...
    it("카테고리 필터로 occurrence를 조회한다", async () => {
        ...
        expect(
            onlySubs.body.every(
                (o: { expense: { category: string } }) =>
                    o.expense.category === "구독",
            ),
        ).toBe(true)
    })
    ...
    it("합계 응답에 basisCounts와 카테고리/결제수단/상태별 합계가 포함된다", async () => {
        ...
        expect(
            sum.body.byCategory.find((b: { key: string }) => b.key === "카드"),
        ).toBeDefined()
        expect(
            sum.body.byPaymentMethod.find(
                (b: { key: string }) => b.key === "신한카드",
            ),
        ).toBeDefined()
        expect(
            sum.body.byStatus.find((b: { key: string }) => b.key === "PAID"),
        ).toBeDefined()
