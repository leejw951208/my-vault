        ...
        try {
            await this.prisma.$transaction(async (tx) => {
                ...
                for (const entry of entries) {
                    const body = this.crypto.openJson<Record<string, unknown>>(
                        currentKey,
                        {
                            iv: Buffer.from(entry.iv),
                            ciphertext: Buffer.from(entry.ciphertext),
    ...
    async importContainer(
        ...
        try {
            container = JSON.parse(
                containerBuf.toString("utf8"),
            ) as ExportContainer
        } catch {
            throw new BadRequestException({
                code: VAULT_ERRORS.IMPORT_CORRUPT,
                message: "export 파일을 해석할 수 없습니다.",
            })
        }

        if (
            container?.magic !== EXPORT_MAGIC ||
            typeof container.version !== "number"
        ) {
            throw new BadRequestException({
                code: VAULT_ERRORS.IMPORT_CORRUPT,
                message: "컨테이너 매직/버전이 잘못되었습니다.",
            })
        }

        const salt = Buffer.from(container.kdf.salt, "base64")
        const importKey = await this.crypto.deriveKey(
            normalizeMaster(rawMaster),
            salt,
            {
        ...
        return this.prisma.$transaction(async (tx) => {
            ...
            for (const item of decoded.entries) {
                ...
                if (existing) {
                    if (mode === "replace") {
                        const sealed = this.crypto.sealJson(
                            sessionKey,
                            item.body,
                        )
                        await tx.vaultEntry.update({
