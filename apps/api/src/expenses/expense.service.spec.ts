    ...
    expenseOccurrence = {
        ...
        deleteMany: async ({
            ...
            this.occurrences = this.occurrences.filter((o) => {
                if (where.expenseId && o.expenseId !== where.expenseId)
                    return true
                if (where.status && o.status !== where.status) return true
                if (where.dueDate?.gte && o.dueDate < where.dueDate.gte)
                    return true
                return false
            })
...
describe("ExpenseService", () => {
    ...
    it("수정 시 PAID 인스턴스는 보존하고 오늘 이후 SCHEDULED만 재생성한다", async () => {
        ...
        expect(futureScheduled.every((o) => o.expectedAmount === 60000)).toBe(
            true,
        )
    })
    ...
    it("삭제 시 isActive=false로 두고 오늘 이후 SCHEDULED만 제거한다", async () => {
        ...
        expect(prisma.occurrences.some((o) => o.status === "SCHEDULED")).toBe(
            false,
        )
    })
    ...
    it("update에서 dayOfMonth가 범위 밖이면 BadRequestException", async () => {
        ...
        await expect(
            service.update(expense.id, { dayOfMonth: 99 }),
        ).rejects.toThrow("결제일")
    })
