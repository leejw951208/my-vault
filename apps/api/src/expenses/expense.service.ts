...
function validateRecurrenceRule(rule: {
  ...
  if (rule.recurrence === 'MONTHLY') {
    if (rule.dayOfMonth === null || rule.dayOfMonth === undefined) {
      throw new BadRequestException('매월 반복은 결제일(dayOfMonth)이 필요합니다.');
    }
    if (rule.dayOfMonth < 1 || rule.dayOfMonth > 31) {
      throw new BadRequestException('결제일은 1~31 사이의 정수여야 합니다.');
    }
  } else if (rule.recurrence === 'WEEKLY') {
    if (rule.dayOfWeek === null || rule.dayOfWeek === undefined) {
      throw new BadRequestException('매주 반복은 요일(dayOfWeek)이 필요합니다.');
    }
    ...
    if (rule.dayOfMonth === null || rule.dayOfMonth === undefined) {
      throw new BadRequestException('매년 반복은 결제일(dayOfMonth)이 필요합니다.');
    }
    if (rule.dayOfMonth < 1 || rule.dayOfMonth > 31) {
      throw new BadRequestException('결제일은 1~31 사이의 정수여야 합니다.');
    }
    if (rule.monthOfYear === null || rule.monthOfYear === undefined) {
      throw new BadRequestException('매년 반복은 월(monthOfYear)이 필요합니다.');
    }
...
export class ExpenseService {
  ...
  async findById(id: string) {
    const expense = await this.prisma.recurringExpense.findUnique({ where: { id } });
    if (!expense) {
  ...
  async create(dto: CreateExpenseDto) {
    ...
    if (endDate && endDate < startDate) {
      throw new BadRequestException('종료일은 시작일과 같거나 그 이후여야 합니다.');
    }
  ...
  async update(id: string, dto: UpdateExpenseDto) {
    ...
    const startDate = dto.startDate ? parseIsoDate(dto.startDate) : existing.startDate;
