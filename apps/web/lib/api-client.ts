...
export async function createExpense(input: CreateExpenseInput): Promise<RecurringExpense> {
  const { data } = await apiClient.post<RecurringExpense>('/expenses', input);
  return data;
}

export async function updateExpense(id: string, input: UpdateExpenseInput): Promise<RecurringExpense> {
  const { data } = await apiClient.patch<RecurringExpense>(`/expenses/${id}`, input);
  return data;
}
...
export async function getOccurrences(params: ListOccurrencesParams = {}): Promise<ExpenseOccurrence[]> {
  const { data } = await apiClient.get<ExpenseOccurrence[]>('/occurrences', { params });
  return data;
}
...
export async function updateOccurrence(
  ...
  const { data } = await apiClient.patch<ExpenseOccurrence>(`/occurrences/${id}`, input);
  return data;
}

export async function getSummary(from: string, to: string): Promise<SummaryResponse> {
  const { data } = await apiClient.get<SummaryResponse>('/summary', { params: { from, to } });
  return data;
}
