...
export function OccurrencePanel({ occurrence, onClose, onUpdated }: Props) {
    const [actualInput, setActualInput] = useState<string>(
        occurrence.actualAmount !== null
            ? occurrence.actualAmount.toLocaleString("ko-KR")
            : "",
    )
        ...
        try {
            ...
            if (status === "PAID") {
                payload.actualAmount =
                    parseAmount(actualInput) ?? occurrence.expectedAmount
            }
    ...
    return (
        <div className="card inline-bottom-sheet" style={{ marginTop: 16 }}>
            ...
            <div className="muted">
                {formatDate(occurrence.dueDate)} · {occurrence.expense.category}
            </div>
            <div style={{ marginTop: 8 }}>
                예상{" "}
                <span className="amount">
                    {formatCurrency(expected, occurrence.expense.currency)}
                </span>
                {actual !== null && (
                    <>
                        {" · 실제 "}
