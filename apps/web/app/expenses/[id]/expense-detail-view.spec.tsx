...
describe('ExpenseDetailView view↔edit 토글', () => {
  ...
  it('수정 버튼을 누르면 폼이 열리고, 저장하면 view 로 복귀하며 새 값이 반영된다', async () => {
    updateExpenseMock.mockResolvedValue(makeExpense({ name: '넷플릭스 프리미엄', amount: 19000 }));

    render(<ExpenseDetailView initial={makeExpense()} />);

    expect(screen.getByRole('heading', { level: 1 }).textContent).toBe('넷플릭스');
    ...
    expect(updateExpenseMock).toHaveBeenCalledWith('exp-1', expect.objectContaining({ name: '넷플릭스' }));
    expect(screen.queryByRole('button', { name: '저장' })).toBeNull();
    expect(screen.getByRole('heading', { level: 1 }).textContent).toBe('넷플릭스 프리미엄');
  });
